...
import classnames.ClassNames.getExpectedFast;
		...
		describe("Npm lib tests for classNames() with fast()", {
			it("keeps object keys with truthy values", {
				ClassNames.fast({
					...
					d: null,
					e: 1
				}).should.be(getExpectedFast("a e"));
			});

			it("joins arrays of class names and ignore falsy values", {
				ClassNames.fast("a", 0, null, true, 1, "b")
				.should.be(getExpectedFast("a 1 b"));
			});

			it("supports heterogenous arguments", {
				ClassNames.fast({a: true}, "b", 0)
				.should.be(getExpectedFast("a b"));
			});

			it("should not repeat spaces", {
				ClassNames.fast("", "b", {}, "").should.be(getExpectedFast("b"));
			});

			it("returns an empty string for an empty configuration", {
				ClassNames.fast({}).should.be(getExpectedFast(""));
			});

			it("supports an array of class names", {
				ClassNames.fast(["a", "b"]).should.be(getExpectedFast("a b"));
			});

			it("joins array arguments with string arguments", {
				ClassNames.fast(["a", "b"], "c").should.be(getExpectedFast("a b c"));
				ClassNames.fast("c", ["a", "b"]).should.be(getExpectedFast("c a b"));
			});

			it("handles multiple array arguments", {
				ClassNames.fast(["a", "b"], ["c", "d"]).should.be(getExpectedFast("a b c d"));
			});
			...
			it("handles arrays that include objects", {
				ClassNames.fast(["a", {b: true, c: false}]).should.be(getExpectedFast("a b"));
			});

			it("handles deep array recursion", {
				ClassNames.fast(["a", ["b", ["c", {d: true}]]]).should.be(getExpectedFast("a b c d"));
			});

			it("handles arrays that are empty", {
				ClassNames.fast("a", []).should.be(getExpectedFast("a"));
			});
			...
			it("handles all types of truthy and falsy property values as expected", {
				...
				.should.be(getExpectedFast("nonEmptyString whitespace function emptyObject nonEmptyObject emptyList nonEmptyList greaterZero"));
			});
		});
	}
